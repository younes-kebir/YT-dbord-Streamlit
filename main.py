# -*- coding: utf-8 -*-
"""
Created on Mon Apr 18 21:13:21 2022
...
def load_data(): 
    ...
    df_com = pd.read_csv('All_Comments_Final.csv')
    df_perf = pd.read_csv('Video_Performance_Over_Time.csv')

    df_perf['Date'] = pd.to_datetime(df_perf['Date'])
    
    return df_agg, df_agg_sub, df_com, df_perf
# load data 
df_agg, df_agg_sub, df_com, df_perf = load_data()
...
df_perf_diff = pd.merge(df_perf, df_agg.loc[:,['Video','Video publish time']], left_on ='External Video ID', right_on = 'Video')
df_perf_diff['days_published'] = (df_perf_diff['Date'] - df_perf_diff['Video publish time']).dt.days

perf_12mo = df_agg['Video publish time'].max() - pd.DateOffset(months =12)
df_perf_diff_yr = df_perf_diff[df_perf_diff['Video publish time'] >= perf_12mo]

views_days = pd.pivot_table(df_perf_diff_yr,index= 'days_published',values ='Views', aggfunc = [np.mean,np.median,lambda x: np.percentile(x, 80),lambda x: np.percentile(x, 20)]).reset_index()
...
if add_sidebar == 'Individual Video Analysis' : 
    ...
    agg_perf_filtered = df_perf_diff[df_perf_diff['Video Title'] == video_select]
    first_30 = agg_perf_filtered[agg_perf_filtered['days_published'].between(0,30)]
